...
//        }
    ...
    func getCroppedImageRectForTile(gridSize : CGSize, gridPt : CGPoint, tileLoc : TileCoordinate, zoom : UInt, offset : CGPoint) -> CGRect {
        ...
        let tileWidth = tileLoc.northWest.distance(from: CLLocationCoordinate2D(latitude: tileLoc.northWest.latitude, longitude: tileLoc.southEast.longitude))
        let tileHeight = tileLoc.northWest.distance(from: CLLocationCoordinate2D(latitude: tileLoc.southEast.latitude, longitude: tileLoc.northWest.longitude))
        let convertedTileWidth = tileWidth / self.metersPerPx
        let convertedTileHeight = tileHeight / self.metersPerPx

//        let northWestTilePt = mapView.projection.point(for: tileLoc.northWest)
//        let southEastTilePt = mapView.projection.point(for: tileLoc.southEast)
//
//        let northWestPt = mapView.projection.point(for: boundaryQuad.northWest)
//        let northEastPt = mapView.projection.point(for: boundaryQuad.northEast)
//        let southWestPt = mapView.projection.point(for: boundaryQuad.southWest)
//        let southEastPt = mapView.projection.point(for: boundaryQuad.southEast)
//
//        let imageXLoc = Double(northWestPt.x - northWestTilePt.x) / self.metersPerPx
//        let imageYLoc = Double(northWestPt.y - northWestTilePt.y) / self.metersPerPx
//
//        let xWidth = (Double(southEastTilePt.x) / self.metersPerPx) - imageXLoc
//        let yHeight = (Double(southEastTilePt.y) / self.metersPerPx) - imageYLoc

        
        // Distance from left edge of tile, to left edge of image
        let convertedNWCorner = CLLocationCoordinate2D(latitude: tileLoc.northWest.latitude, longitude: boundaryQuad.northWest.longitude)
        let xDistanceFromCorner = tileLoc.northWest.distance(from: convertedNWCorner)// - offset.x
        let convertedSECorner = CLLocationCoordinate2D(latitude: boundaryQuad.southEast.latitude, longitude: tileLoc.southEast.longitude)
        let yDistanceFromCorner = tileLoc.southEast.distance(from: convertedSECorner)// - offset.y
        ...
        let convertedRect = CGRect(x: convertedXValue, y: convertedYValue, width: min(Double(self.image!.size.width), convertedTileWidth), height: min(Double(self.image!.size.height), convertedTileHeight))
        ...
        var imageHeight = convertedTileHeight - convertedYValue
        var imageWidth = convertedTileWidth - convertedYValue
        if (self.image!.size.height + CGFloat(convertedYValue)) < CGFloat(convertedTileHeight) {
        ...
        let cropped1 = currentImage.cropped(boundingBox: convertedRect)

        return convertedRect
    }
    
    /*
     THIS WORKS FOR 1,2 grid
     {
     DispatchQueue.main.sync {
     ...
     guard let cropped = currentImage.cropped(boundingBox: imageRect) else {
     return nil
     }
